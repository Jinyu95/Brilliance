...
# ══════════════════════════════════════════════════════════════════════════════
# Multi-objective analysis — Pareto frontier, brightness, scaling laws
# ══════════════════════════════════════════════════════════════════════════════


def pareto_dominates(a: dict[str, Any], b: dict[str, Any],
                     objectives: list[str],
                     directions: list[str] | None = None) -> bool:
    """Check whether design *a* Pareto-dominates design *b*.

    A design *a* dominates *b* when it is at least as good on ALL objectives
    and strictly better on at least one.  "Better" is defined by `directions`:
    ``"min"`` (lower is better) or ``"max"`` (higher is better).

    Parameters
    ----------
    a, b : dict
        Design result dictionaries with objective values.
    objectives : list[str]
        Keys to compare (e.g. ``["emittance", "circumference"]``).
    directions : list[str] | None
        ``"min"`` or ``"max"`` for each objective.  If None, all are ``"min"``.
    ...
    for obj, direction in zip(objectives, directions):
        va = _to_float(a.get(obj))
        vb = _to_float(b.get(obj))
...
def compute_pareto_front(results: list[dict[str, Any]],
                         objectives: list[str],
                         directions: list[str] | None = None) -> list[dict[str, Any]]:
    """Extract the Pareto-optimal subset from a list of design results.

    Uses the naive O(n²) algorithm — fine for up to ~10⁴ designs.

    Parameters
    ----------
    results : list[dict]
        Design result dictionaries; each must have objective values and ideally
        a ``"stable"`` key.  Unstable designs are excluded.
    objectives : list[str]
        Keys to consider for Pareto optimality.
    directions : list[str] | None
        ``"min"`` or ``"max"`` per objective.  Default: all ``"min"``.
    ...
    # Exclude unstable / error designs
    viable = [
        r for r in results
    ...
    for candidate in viable:
        ...
        for existing in front:
            if pareto_dominates(existing, candidate, objectives, directions):
                dominated = True
                break
            if pareto_dominates(candidate, existing, objectives, directions):
                # candidate dominates an existing member → remove it
                front = [f for f in front if f is not existing]
        if not dominated:
            front.append(candidate)

    # Sort by the first objective
    reverse_sort = (directions[0] == "max")
    try:
        front.sort(key=lambda r: _to_float(r.get(objectives[0], 0.0)) or 0.0,
...
def compute_brightness_fom(result: dict[str, Any],
                           beam_current_ma: float = 200.0) -> float | None:
    """Compute a brightness figure of merit for a storage-ring design.

    Uses the standard spectral-brightness proxy for electron storage rings:

        B ∝ I_beam / (4π² · ε_x · ε_y)

    where I_beam is the average beam current (set here in mA, converted
    internally to A), and ε_x, ε_y are the natural emittances in m·rad.

    For rings where ε_y is not reported (common — vertical emittance is
    typically ε_x × coupling_κ with κ ~ 0.01 → 0.001), we assume the
    coupling-dominated floor and report the horizontal-emittance proxy:

        B_proxy = I_beam / ε_x

    which is proportional to the diffraction-limited brightness in the
    horizontal plane.

    Parameters
    ----------
    result : dict
        Must contain at minimum ``"emittance"`` (ε_x in m·rad).
        May also contain ``"emittance_y"`` or ``"emitty"`` for the vertical plane.
    beam_current_ma : float
        Average beam current in mA.  Default 200 mA (typical for MBA rings).

    Returns
    -------
    float | None
        Brightness FoM in A/(m·rad) [if only ε_x is known] or A/(m²·rad²)
        [if both ε_x and ε_y are known].  Returns None if emittance is missing.
    """
    emit_x = _to_float(result.get("emittance"))
    if emit_x is None or not math.isfinite(emit_x) or emit_x <= 0:
        return None

    current_A = beam_current_ma * 1e-3  # mA → A

    emit_y = _to_float(result.get("emittance_y") or result.get("emitty"))
    if emit_y is not None and math.isfinite(emit_y) and emit_y > 0:
        # 2D brightness proxy in A/(m²·rad²)
        # NOTE: the factor 1/(4π²) is dropped here — this FoM is proportional
        # to the true spectral brightness, not equal to it.  We only need
        # consistent ranking for Pareto analysis.
        return current_A / (emit_x * emit_y)
    else:
        # 1D proxy in A/(m·rad) — proportional to brightness at the
        # diffraction limit when coupling is negligible.
        return current_A / emit_x


# ── Empirical scaling-law analysis ────────────────────────────────────────────

# Physical constants (Sands / Wiedemann)
_Cq = 3.8319e-13       # m  — quantum diffusion constant
_Cgamma = 8.85e-5      # m·GeV⁻³  — energy-loss constant
_M_E_EV = 0.51099895069e6   # eV  — electron rest energy
...
def theoretical_emittance_scaling(
    energy_gev: float,
    n_cells: int,
    n_bends_per_cell: int,
    lattice_factor_F: float,
) -> float:
    """Compute the analytical emittance from the textbook scaling law.

    ε₀ = Cq · γ² · θ³ · F

    where
        γ    = E / m_e c²           Lorentz factor
        θ    = 2π / (n_cells · n_bends_per_cell)   bend angle per dipole [rad]
        F    = dimensionless lattice factor
        Cq   = 3.8319 × 10⁻¹³ m    quantum diffusion constant

    This is the *theoretical floor* — actual emittance is always higher due to
    non-optimal optics and realistic constraints.
    ...
    n_bends_per_cell : int
        Number of main dipoles per cell.
    lattice_factor_F : float
        Dimensionless lattice factor (FODO ≈ 0.144, DBA ≈ 0.022, 7BA ≈ 0.0046).

    Returns
    -------
    float
        Theoretical minimum emittance in m·rad.
    """
    gamma = energy_gev * 1e9 / _M_E_EV
    theta = 2.0 * math.pi / (n_cells * n_bends_per_cell)
    return _Cq * (gamma ** 2) * (theta ** 3) * lattice_factor_F
...
def fit_scaling_exponent(
    ...
    *,
    ...
    dependent_var: str = "emittance",
) -> dict[str, Any]:
    """Fit the empirical scaling exponent of emittance vs. a design parameter.

    The analytical theory predicts:

        ε ∝ n_cells^{-3}          (at fixed energy, fixed bends/cell)
        ε ∝ γ²                    (at fixed cell geometry)
        ε ∝ θ³                    (θ = 2π/(n_cells · n_bends/cell))

    This function fits log(ε) = α + β·log(independent_var) using simple
    least-squares and reports whether β matches the theoretical expectation.

    Parameters
    ----------
    results : list[dict]
        Design result dicts, each containing ``dependent_var``, ``independent_var``,
        and ideally ``"energy_gev"``, ``"stable"``.
    independent_var : str
        Variable to fit against — typically ``"n_cells"``, ``"theta_mrad"``,
        or ``"energy_gev"``.
    dependent_var : str
        Variable to fit — typically ``"emittance"``.

    Returns
    -------
    dict with keys:
        exponent_beta       : fitted slope β
        intercept_alpha     : fitted intercept α
        ...
        theoretical_exponent : expected β from analytical theory (or None)
        deviation_sigma     : (β_observed − β_theory) / σ_beta, or None
        note                : interpretation
    """
    import math as _m
    ...
    for r in results:
        ...
        y = _to_float(r.get(dependent_var))
        if x is None or y is None:
            continue
        if not _m.isfinite(x) or not _m.isfinite(y) or x <= 0 or y <= 0:
            continue
        xs.append(_m.log(x))
        ys.append(_m.log(y))
    ...
    if n < 3:
        return {
            ...
            "note": f"Insufficient data points (n={n}, need ≥ 3 stable designs)",
        }

    # Simple least-squares:  β = Σ(x_i - x̄)(y_i - ȳ) / Σ(x_i - x̄)²
    x_mean = sum(xs) / n
    y_mean = sum(ys) / n
    ...
    if ss_xx == 0:
        return {
            ...
            "theoretical_exponent": None,
            "deviation_sigma": None,
            "note": "Independent variable has zero variance",
        }
    ...
    # R²
    ss_res = sum(
        (yi - (alpha + beta * xi)) ** 2 for xi, yi in zip(xs, ys)
    )
    ss_tot = sum((yi - y_mean) ** 2 for yi in ys)
    r_squared = 1.0 - ss_res / ss_tot if ss_tot > 0 else 0.0

    # Standard error on beta
    sigma2 = ss_res / (n - 2) if n > 2 else float("inf")
    se_beta = _m.sqrt(sigma2 / ss_xx) if _m.isfinite(sigma2) and ss_xx > 0 else None

    # Theoretical expectation
    _theory_map = {
        "n_cells": -3.0,           # ε ∝ θ³ ∝ 1/n_cells³
        "n_bends_per_cell": -3.0,  # ε ∝ θ³ ∝ 1/n_bends³
        "energy_gev": 2.0,         # ε ∝ γ² ∝ E²
        "theta_mrad": 3.0,         # ε ∝ θ³
    }
    theory = _theory_map.get(independent_var)

    deviation_sigma = None
    if beta is not None and theory is not None and se_beta is not None and se_beta > 0:
        deviation_sigma = (beta - theory) / se_beta

    note_parts = []
    if theory is not None and beta is not None:
        note_parts.append(
            f"Empirical β={beta:.4f} vs. theoretical {theory:.1f} "
            f"(Δ={beta - theory:.4f})"
        )
        if deviation_sigma is not None and abs(deviation_sigma) > 2.0:
            note_parts.append(
                f"Deviation is {abs(deviation_sigma):.1f}σ — statistically significant. "
                f"This suggests the lattice factor F has a non-trivial dependence on "
                f"{independent_var} beyond the constant-F assumption."
            )
        elif deviation_sigma is not None:
            note_parts.append(
                f"Deviation is {abs(deviation_sigma):.1f}σ — consistent with "
                f"the theoretical scaling within uncertainty."
            )
    if r_squared is not None and r_squared < 0.5:
        note_parts.append("Low R² — substantial scatter; check for confounding variables.")
...
def measure_lattice_factor_F(result: dict[str, Any]) -> float | None:
    """Back out the empirical lattice factor F from a design result.

    From ε₀ = Cq · γ² · θ³ · F, we solve for:

        F_empirical = ε₀ / (Cq · γ² · θ³)

    This is the *effective* lattice factor — the dimensionless constant that
    would be needed for the analytical formula to reproduce the computed
    emittance exactly.  Real (non-ideal) lattices have F_empirical > F_theory.

    Parameters
    ----------
    result : dict
        Must contain ``"emittance"``, ``"energy_gev"``, and either
        ``"n_cells"`` + ``"n_bends_per_cell"`` or ``"theta_mrad"``.

    Returns
    -------
    float | None
        Empirical lattice factor F, or None if required data are missing.
    """
    emit = _to_float(result.get("emittance"))
    e_gev = _to_float(result.get("energy_gev"))
    if emit is None or e_gev is None:
        return None
    if not all(math.isfinite(v) and v > 0 for v in (emit, e_gev)):
        return None

    gamma = e_gev * 1e9 / _M_E_EV

    theta_mrad = _to_float(result.get("theta_mrad"))
    if theta_mrad is None or not math.isfinite(theta_mrad) or theta_mrad <= 0:
        n_cells_val = result.get("n_cells")
        n_bends = result.get("n_bends_per_cell")
        if n_cells_val is None or n_bends is None:
            return None
        try:
            nc = int(n_cells_val)
            nb = int(n_bends)
        ...
        if nc <= 0 or nb <= 0:
            return None
        theta = 2.0 * math.pi / (nc * nb)
        theta_mrad_float = theta * 1e3
    else:
        theta = theta_mrad * 1e-3
        theta_mrad_float = float(theta_mrad)

    denominator = _Cq * (gamma ** 2) * (theta ** 3)
    if denominator <= 0:
        return None
    F = emit / denominator
...
def compute_robinson_sum(result: dict[str, Any]) -> float | None:
    """Extract the Robinson damping partition sum Jx + Jy + Je from a result.

    Returns None if any partition number is missing.  The Robinson theorem
    requires Jx + Jy + Je = 4 for any planar ring without transverse coupling.
    """
    Jx = _to_float(result.get("Jx"))
    Jy = _to_float(result.get("Jy"))
    Je = _to_float(result.get("Je"))
    if None in (Jx, Jy, Je):
        return None
    if not all(math.isfinite(v) for v in (Jx, Jy, Je)):
        return None
    return float(Jx) + float(Jy) + float(Je)


def compute_U0_crosscheck(result: dict[str, Any]) -> dict[str, Any] | None:
    """Cross-check the energy loss per turn U0 against the analytical formula.

    The analytical formula for electron storage rings is:

        U₀ = C_γ · E⁴ · I₂ / (2π)   [eV]

    where C_γ = 8.85×10⁻⁵ m·GeV⁻³ and I₂ is the second radiation integral.

    Returns a dict with the reported U0 (from ringpara), the analytical U0,
    and the relative error.  Returns None if I₂ or energy is missing.
    """
    U0_reported = _to_float(result.get("U0_eV"))
    I2 = _to_float(result.get("I2"))
    e_gev = _to_float(result.get("energy_gev"))

    if I2 is None or e_gev is None:
        return None
    if not all(math.isfinite(v) and v > 0 for v in (I2, e_gev)):
        return None

    U0_analytic = _Cgamma * (e_gev ** 4) * I2 / (2.0 * math.pi) * 1e9  # → eV

    rel_err = None
    if U0_reported is not None and math.isfinite(U0_reported) and U0_analytic > 0:
        rel_err = (U0_reported - U0_analytic) / U0_analytic

    return {
        "U0_reported_eV": U0_reported,
        "U0_analytic_eV": U0_analytic,
        "relative_error": rel_err,
        "formula_valid": U0_reported is not None
                          and U0_reported > 0
                          and rel_err is not None
                          and abs(rel_err) < 0.15,
    }
